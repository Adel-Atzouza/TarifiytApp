...
class Container(containers.DeclarativeContainer):
    wiring_config = containers.WiringConfiguration(packages=[interactors])

    lessons_repository_logic = providers.Factory(LessonsRepositoryLogic)


LessonsRepositoryLogicDep = Annotated[
    LessonsRepositoryLogic, Depends(Provide[Container.lessons_repository_logic])
]
